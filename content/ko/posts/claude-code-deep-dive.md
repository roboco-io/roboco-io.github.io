...
- Claude Code의 강점은 하나의 거대한 프롬프트가 아니라, 상황에 따라 조립되는 프롬프트 시스템에서 나온다.
- CLAUDE.md, 시스템 리마인더, 서브 에이전트, 권한 검증은 각각 컨텍스트 품질과 안전성을 높이는 역할을 맡는다.
- 핵심 교훈은 프롬프트를 잘 쓰는 수준을 넘어, 컨텍스트와 작업 흐름을 아키텍처로 설계해야 한다는 것이다.
