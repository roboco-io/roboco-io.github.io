...
- 페르소나 지정은 모델의 출력 분포를 특정 역할 쪽으로 조향하지만, 그 역할이 명시적 제약으로 남지는 않는다.
- 메타인지 방식은 자기 평가를 토큰으로 먼저 생성하게 해 이후 답변을 더 신중하고 일관된 방향으로 제한한다.
- 자신감 있는 전문가 톤이 필요하면 페르소나를, 불확실성과 한계 인식이 중요하면 메타인지 방식을 쓰는 편이 적합하다.
