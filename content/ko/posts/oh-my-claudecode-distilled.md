...
- OMC는 Claude Code의 서브 에이전트, 스킬, 훅을 묶어 자연어 요청만으로 필요한 작업 모드를 자동 조합하는 플러그인이다.
- 핵심 가치는 명령어 학습 부담을 줄이고, 계획·병렬화·리서치·디자인 감각 같은 워크플로를 Claude Code 안에서 자연스럽게 활성화하는 데 있다.
- 특히 복잡한 개발 작업을 역할별로 나누고 끝까지 실행해야 하는 상황에서 생산성을 높여준다.
