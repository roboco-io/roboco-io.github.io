...
- 한국어와 영어 중 어느 언어가 더 좋은 결과를 내느냐보다 중요한 것은 토큰 효율이다.
- 한국어는 같은 내용을 전달할 때 더 많은 토큰을 쓸 수 있어 비용, 속도, 전달 가능한 컨텍스트에 영향을 줄 수 있다.
- 대규모 프로젝트나 규칙 파일처럼 반복 입력되는 문서는 영어로 간결하게 관리하는 전략을 고려할 만하다.
