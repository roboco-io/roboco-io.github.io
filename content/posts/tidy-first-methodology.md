...
- Tidy First는 구조적 변화와 행동적 변화를 분리해 AI와 함께 코딩할 때 복잡도를 낮추는 접근법이다.
- 먼저 코드를 정리하고 테스트로 검증한 뒤 기능 변경을 넣어야 작업 흐름이 흔들리지 않는다.
- 클로드 코드 같은 도구와 함께 쓰면 개발자가 코드 품질과 방향에 대한 주도권을 유지하기 쉽다.
