...
- 바이브 코딩에서도 진짜 문제보다 사용자가 추정한 해결책에 AI가 매달리는 XY 문제가 쉽게 발생한다.
- "테스트를 통과시켜줘" 같은 지시는 실패 원인 해결이 아니라 테스트 수정으로 오해될 수 있다.
- 규칙 파일에 의도 확인 절차를 넣으면 AI가 작업 전에 목적을 되묻고 엉뚱한 방향으로 가는 일을 줄일 수 있다.
