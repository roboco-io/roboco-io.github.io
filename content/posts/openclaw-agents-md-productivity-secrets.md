...
- OpenClaw의 생산성은 여러 AI 에이전트를 병렬로 돌리는 것만이 아니라, `AGENTS.md`에 작업 규칙을 명확히 계약한 데서 나온다.
- 좋은 `AGENTS.md`는 코드 구조, 임포트 경계, 빌드 게이트, 멀티 에이전트 안전 규칙을 구체적으로 적는다.
- 에이전트에게 설명보다 금지 규칙과 검증 절차를 주면 반복 실수와 충돌을 줄일 수 있다.

---

## 1. 에이전트의 인지 지도를 만들어라

AGENTS.md의 첫 번째 섹션은 [Project Structure & Module Organization](https://github.com/openclaw/openclaw/blob/main/AGENTS.md#project-structure--module-organization)이다. 사람이 온보딩할 때 "이 코드는 어디에 있어?"가 첫 질문이듯, AI 에이전트에게도 코드베이스의 지도가 필요하다.
...
OpenClaw의 가장 독특한 생산성 비밀은 [Collaboration / Safety Notes](https://github.com/openclaw/openclaw/blob/main/AGENTS.md#collaboration--safety-notes) 섹션의 **멀티 에이전트 안전(multi-agent safety)** 규칙이다. Steinberger는 3~8개의 에이전트를 동시에 같은 저장소에서 실행한다. 이것이 가능한 이유는 에이전트 간 충돌을 방지하는 명확한 규칙 때문이다.
...
AGENTS.md의 [Coding Style & Naming Conventions](https://github.com/openclaw/openclaw/blob/main/AGENTS.md#coding-style--naming-conventions) 섹션은 일반적인 스타일 가이드와 성격이 다르다. "탭 vs 스페이스" 같은 취향이 아니라, 에이전트가 만들어내는 **구조적 결함을 차단하는 안전장치**다.
...
AGENTS.md의 [Release / Advisory Workflows](https://github.com/openclaw/openclaw/blob/main/AGENTS.md#release--advisory-workflows) 섹션은 세 가지 전문 스킬(skill)을 참조한다:
...
AGENTS.md의 [Security & Configuration Tips](https://github.com/openclaw/openclaw/blob/main/AGENTS.md#security--configuration-tips)와 [Testing Guidelines](https://github.com/openclaw/openclaw/blob/main/AGENTS.md#testing-guidelines) 섹션은 보안과 릴리스에 관한 **절대 규칙**을 담고 있다.
